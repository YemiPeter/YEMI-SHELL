...
PillSurface {
    ...
    readonly property point caretPoint: {
        void root.width;
        void root.height;
        if (!search || !search.input) return Qt.point(0, 0);
    ...
    function mapCategory(raw) {
        const order = [
            ["TerminalEmulator", "Terminal"], ["WebBrowser", "Browser"], ["InstantMessaging", "Chat"],
            ["Audio", "Media"], ["AudioVideo", "Media"], ["Video", "Media"], ["Game", "Game"],
            ["Development", "Dev"], ["Graphics", "Graphics"], ["Office", "Office"],
            ["Settings", "System"], ["System", "System"], ["Utility", "Tool"], ["Network", "Net"]
        ];
        const cats = String(raw).split(/[;,]/);
        for (let i = 0; i < order.length; i++)
            if (cats.includes(order[i][0])) return order[i][1];
        return "";
    }
    ...
    Component.onCompleted: {
        ...
        try { root.usage = raw && raw.length ? JSON.parse(raw) : ({}); }
        catch (e) { root.usage = ({}); }
    }

    implicitHeight: col.implicitHeight + 20 * s
    implicitWidth: 360 * s
    ...
    Column {
        ...
        ListView {
            id: list
            width: parent.width
            height: Math.min(240 * root.s, model * 34 * root.s)
            ...
            delegate: Item {
                ...
                readonly property string secondary: {
                    ...
                    if (entry.categories && entry.categories.length > 0) return root.mapCategory(entry.categories);
                    return "";
                }
