...
Singleton {
    ...
    // === Dark/Light Mode ===
    // Reads from ~/.config/quickshell/state/colormode
    // Updated by toggle-colormode.sh and after-wall.sh
    property bool isLightMode: false

    FileView {
        id: colormodeFile
        path: `${Quickshell.env("HOME")}/.config/quickshell/state/colormode`
        watchChanges: true
        onLoaded: root.isLightMode = (text().trim() === "light")
        onFileChanged: root.isLightMode = (text().trim() === "light")
    }
    ...
    // Surface colors — mode-aware
    // Dark mode: lighten the dark background to create elevation
    // Light mode: darken the light background to create elevation
    readonly property color surface: background
    readonly property color surfaceDim: isLightMode
        ? Qt.darker(background, 1.05)
        : Qt.darker(background, 1.1)
    readonly property color surfaceBright: isLightMode
        ? Qt.darker(background, 0.88)
        : Qt.lighter(background, 1.3)
    readonly property color surfaceContainer: isLightMode
        ? Qt.darker(background, 1.08)
        : Qt.lighter(background, 1.15)
    readonly property color surfaceContainerLow: isLightMode
        ? Qt.darker(background, 1.04)
        : Qt.lighter(background, 1.08)
    readonly property color surfaceContainerHigh: isLightMode
        ? Qt.darker(background, 1.12)
        : Qt.lighter(background, 1.22)
    readonly property color surfaceContainerHighest: isLightMode
        ? Qt.darker(background, 1.18)
        : Qt.lighter(background, 1.3)
