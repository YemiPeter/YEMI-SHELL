...
| 3 | ✅ **DONE** — **Pick ONE color engine.** Took option (b): skwd v2's built-in theming is disabled (`theme.policy = "off"`) and `after-wall.sh` stays the only writer. See §6 | One palette, no races, no flicker |
...
## 6. RESOLVED — skwd built-in theming disabled (§4 step 3, option **b**)

**Date:** 2026-09-22
**Decision:** YemiShell (`after-wall.sh`) is the sole theming authority. skwd v2 keeps
painting wallpapers but no longer generates or writes any colour data.

### 6.1 The switch

skwd v2 exposes a theme **policy** in `~/.config/skwd-wall-v2/config.json`. Valid
policy values (extracted from `/usr/bin/skwd-walld`):

| Value | Meaning |
|---|---|
| `auto` | Follow the wallpaper automatically |
| `wallpaper` | Re-theme on every wallpaper change |
| `fixed` | Hold a pinned static theme |
| `manual` | Only re-theme on explicit request |
| **`off`** | **Theme engine disabled — writes nothing** |

Key paths (from `skwd_config::keys::theme`):
`theme.policy`, `theme.authority`, `theme.engine`, `theme.mode`, `theme.staticTheme`,
`theme.style`, `theme.scheme`, `theme.savedThemes`.

### 6.2 Applied configuration
...
- `theme.policy = "off"` — stops the matugen/`skwd-iris` backend entirely.
- `theme.authority = "external"` — declares the palette is owned by something outside
  skwd (Quickshell). Valid authorities observed in the binary: `skwd`, `matugen`,
  `noctalia`, `dms`, `caelestia`, `quickshell`, `shell`, `external`, `user`, `manual`,
  `auto`.

A pre-edit backup lives at `~/.config/skwd-wall-v2/config.json.bak`.

### 6.3 Verification (live)

Daemon restart + a real wallpaper change both log:

```
skwd_wall_core::infrastructure::theme::application] theme apply: backend=off dark=true src=...
```

`backend=off` is the confirmation that the theme pipeline short-circuits before matugen
runs. Corroborating evidence — skwd's colour outputs stayed frozen across a wallpaper
change while YemiShell's advanced:

| File | Owner | mtime after change |
|---|---|---|
| `~/.cache/skwd-wall-v2/colors.json` | skwd | **unchanged** (22:02) |
| `~/.cache/skwd-wall-v2/scheme.json` | skwd | **unchanged** (22:02) |
| `~/.cache/skwd-wall-v2/skwd-colors.json` | skwd | **unchanged** (22:02) |
| `~/.cache/yemi-shell/colors.json` | YemiShell | **rewritten** (22:10) |
| `~/.cache/yemi-shell/terminal.json` | YemiShell | **rewritten** (22:10) |
| `~/.cache/yemi-shell/hypr-colors.lua` | YemiShell | **rewritten** (22:10) |

### 6.4 Note — skwd still watches the shell's generated dir

On start-up skwd logs:

```
watching theme provider directory /home/yemi/.local/state/quickshell/user/generated
```

This is **read-only** watcher interest, not a write path: with `policy=off` skwd
consumes nothing from it and produces nothing into it. The stale files in that
directory are leftovers from the earlier `system24`/matugen era (all dated 2026-08-04)
and are not regenerated by either system today.

Consequently `theme.engine` deliberately has no explicit value — `policy=off` is the
gate that matters, and leaving the engine key out avoids clobbering whatever the v2 UI
writes if the user ever flips the policy back on.

### 6.5 Interaction with the rest of the pipeline

Unchanged and still correct:

- `set-wallpaper.sh` → `skwd-helm apply` (skwd remains the **painter**).
- `skwd-wall-sync.sh` watches `skwd-helm watch` to keep the state file in sync when a
  pick comes from skwd's own picker.
- `after-wall.sh` → dominance engine → `~/.cache/yemi-shell/colors.json` is the only
  colour writer.

The two engines no longer race: skwd paints pixels, YemiShell owns colour.
