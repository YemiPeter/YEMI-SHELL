...
// Hover preview for the bar taskbar: a PopupWindow anchored under the hovered
// app button listing that app's windows (icon + title), click to focus.
// Adapted from iNiR's BarTaskbarPreview/BarTaskbarWindowPreview without the
// live window-capture dependency. A ColumnLayout keeps the popup compact even
// when an app has many windows.
PopupWindow {
    id: popup

    // Supplied by Taskbar.qml once loaded (falls back to the singletons).
    ...
    // Close shortly after the cursor leaves the popup itself. The bar's own
    // close timer covers moving back to the app button.
    Timer {
        interval: 200
    ...
    Rectangle {
        ...
        anchors.margins: 6
        radius: 12
        color: popup.theme ? popup.theme.cardBotBase : "#1a1a1a"
        border.width: 1
        ...
        ColumnLayout {
            ...
            Repeater {
                ...
                delegate: Item {
                    ...
                    // Fallback disc while/if the window icon can't be resolved
                    Rectangle {
                        anchors.left: parent.left
                        anchors.leftMargin: 8
                        anchors.verticalCenter: parent.verticalCenter
    ...
    function windowIcon(tl: var): string {
