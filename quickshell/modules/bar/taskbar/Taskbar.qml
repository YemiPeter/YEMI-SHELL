...
Rectangle {
    ...
    // ─── TEMP DEBUG: writes state to /tmp/taskbar_dbg.log ─────────────
    Process {
        id: dbgProc
        running: false
    }
    function dbg(msg: string): void {
        ...
        dbgProc.command = ["sh", "-c", "echo '" + clean + "' >> /tmp/taskbar_dbg.log"]
        dbgProc.running = true
    }
    Component.onCompleted: {
        dbg("__TASKBAR_COMPLETED__ apps=" + root.appItems.length + " vis(init)=" + root.visible)
    }
    onAppItemsChanged: {
        dbg("__APPS__ apps=" + (root.appItems ? root.appItems.length : -1) + " vis=" + root.visible + " niriTls=" + root.compositor.toplevels.length)
    }
}
