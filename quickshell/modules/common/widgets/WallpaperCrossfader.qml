 ...
 *
 * Signals: transitionStarted(), transitionFinished() — external consumers
 * (e.g. color pipeline) can defer work until the swap completes.
 *
 * Renders two Image slots; one is the current wallpaper, the other loads the
 * pending wallpaper. When the pending image is ready, the crossfader animates
 * opacity/position/scale/blur over `transitionDuration` ms with a bezier
 * curve, then promotes the pending slot to active.
 */
Item {
    id: root
    ...
    readonly property bool _canTransition: enableTransitions && _normalizedTransitionType(transitionType) !== "none"
    ...
    // Map awww transition types to the crossfader's internal vocabulary.
    // "simple" and "fade" are both crossfades; directional types become
    // "slide"; "wipe"/"wave" use the wipe path; "grow"/"center"/"outer"
    // become "zoom".
    ...
    Item {
        ...
        Image {
            ...
            layer.enabled: root._canTransition && root._effectiveType === "blurFade"
                           && root._transitioning
