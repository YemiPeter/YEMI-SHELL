...
SettingsSurface {
    ...
    readonly property string monitorsPath: Quickshell.env("RICE_HOME") + "/hypr/modules/monitors.lua"
    readonly property string helper: Quickshell.env("RICE_HOME") + "/hypr/scripts/display-apply.sh"
    ...
    Process {
        ...
        onExited: (code) => {
            if (code === 0) {
                ...
                // Apply failed — nothing changed live, so don't show the
                // confirm-countdown as if a change were pending.
                root.cancelCountdown();
                root.note = "Apply failed — " + out + " " + mode + " was not accepted.";
            }
        }
    }
    ...
    Process {
        ...
        command: ["sh", "-c",
            "niri msg output \"$1\" mode \"$2\" && niri msg output \"$1\" scale \"$3\" && niri msg output \"$1\" position set \"$4\" \"$5\"",
            "sh", out, mode, String(scale), x, y]
        // Refresh the pickers only once the revert has actually landed, so
        // the UI doesn't read the compositor's still-pending state.
        onExited: root.loadMonitorsFromCompositor()
    }
        ...
        onTriggered: {
            ...
            if (root.countdown <= 0) {
                if (Compositor.runningCompositor === "niri" && root.pendingOut.length > 0) {
                    // Revert the temporary Niri output change. The picker
                    // refresh happens in niriRevertProc.onExited, once the
                    // revert has actually landed.
