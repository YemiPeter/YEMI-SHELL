...
SettingsSurface {
    ...
    // Same belt-and-braces reread as the overlay: the label is a binding over the
    // watched file, so refreshing on activation keeps it honest even if a watch
    // notification is missed. Mirrors Look.qml's activation hook.
    onActiveChanged: {
        if (active) {
            decoFile.reload()
        } else {
