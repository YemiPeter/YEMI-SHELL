...
// Ported from iNiR's AppLauncher (services/AppLauncher.qml). Maps logical
// "slots" (terminal, browser, taskManager, bluetooth, network, …) to launch
// commands, with per-slot preset choices persisted via Config.options.apps.
// The iNiR-only app-architecture singletons (ThemeService, MaterialThemeLoader,
// AppLauncher's own UI) were NOT ported; only this launch helper is needed.
Singleton {
    id: root
