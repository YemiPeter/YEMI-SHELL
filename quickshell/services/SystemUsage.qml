...
Singleton {
    ...
    property real cpuTemp: 0  // degrees Celsius
    property var cpuUsageHistory: []  // last 30 cpuPerc values (0-1 range)
    property real memUsed: 0
    property real memTotal: 1
    readonly property real memPerc: memTotal > 0 ? memUsed / memTotal : 0
    property var memoryUsageHistory: []  // last 30 memPerc values (0-1 range)
    property real diskUsed: 0
    property real diskTotal: 1
    readonly property real diskPerc: diskTotal > 0 ? diskUsed / diskTotal : 0

    // Formatted RAM total
    readonly property string maxAvailableMemoryString: root._formatBytes(memTotal)
    ...
    // Helper: KB to GB string (matches iNiR ResourceUsage.kbToGbString)
    ...
    // Lifecycle management (iNiR compatibility)
    function ensureRunning() {
        root.active = true
        if (!updateTimer.running) updateTimer.start()
    }
        ...
        stdout: SplitParser {
            onRead: data => {
                ...
                if (parts.length >= 5) {
                    ...
                    // Track CPU usage history (last 30 readings)
                    cpuUsageHistory.push(cpuPerc)
                    if (cpuUsageHistory.length > 30) cpuUsageHistory.shift()
                }
            }
        }
    }
        ...
        stdout: SplitParser {
            onRead: data => {
                ...
                if (parts.length >= 3) {
                    ...
                    // Track memory usage history (last 30 readings)
                    memoryUsageHistory.push(memPerc)
                    if (memoryUsageHistory.length > 30) memoryUsageHistory.shift()
                }
            }
        }
    }
    ...
    Timer {
        ...
        onTriggered: {
            ...
            // Update CPU temperature every 10s (tickCount % 5 === 0)
            if (tickCount % 5 === 0) {
    ...
    Process {
        id: cpuTempProc
        command: ["/bin/sh", "-c", "cat /sys/class/thermal/thermal_zone*/temp 2>/dev/null | head -1"]
        running: false

        stdout: SplitParser {
            onRead: data => {
                const tempMilli = parseInt(data.trim())
                if (!isNaN(tempMilli) && tempMilli > 0) {
                    cpuTemp = tempMilli / 1000
                }
            }
        }
    }
