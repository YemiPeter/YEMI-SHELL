...
- **Waffle (iNiR clone, Niri-focused) — ported 2026-08-18:**
  ...
  - `modules/waffle/settings/` + `waffleSettings.qml` + `modules/settings/WaffleConfig.qml` copied and **live (2026-08-20)**: the bar's "Unified Settings" action probe-launches `waffleSettings.qml` (toggle via its `waffleSettings` IPC target, else `execDetached`); `onClosing: Qt.quit()` keeps it a standalone process. The 5 iNiR-only singletons (`ThemeService`, `MaterialThemeLoader`, `AppLauncher`, `ShellUpdates`, `Idle`) are ported into `qs.services`, so page imports resolve and the launcher opens.
  - **Current blockers (deferred, not dead):** `services/Wallpapers.qml` is a 9-line stub → `WQuickPage` / `WBackgroundPage` wallpaper picker pages are non-functional until implemented; `WThemesPage` / the `WaffleConfig.qml` THEME section are deferred per §4 THEME DIRECTION.
  - **THEME DECISION REVERSED (2026-08-19):** Waffle is being made a **fully-functional standalone family**, so the 5 iNiR-only singletons (`ThemeService`, `MaterialThemeLoader`, `ShellUpdates`, `Idle`, `YtMusic`) ARE being ported from iNiR into `services/` and registered as `qs.services.*`. (`AppLauncher` was already present in quickshell.) This unblocks the waffle settings UI **and** the left widgets panel's theme/color-scheme features. Ported via copy-only (iNiR stays read-only).
  - **THEME DIRECTION — Waffle uses iNiR theme BYTE-FOR-BYTE, Dominance disconnected (2026-08-20):** Waffle will NOT be bridged to the Dominance/`Dyn` pipeline. Instead it uses iNiR's own theme system verbatim — `Appearance.aurora.*`, `Appearance.angel.*`, `Appearance.inirEverywhere`, `m3colors`, `MaterialThemeLoader`, `ThemeService`, the `Looks.glassActive` (`auroraEverywhere && !Appearance.inirEverywhere`) pipeline, etc. The `WaffleBarContent.qml` stubs (`auroraTransparency: 0.9`, hardcoded `#3a88f2` border) and the `MaterialThemeLoader` guarded no-op are **temporary** — they get restored to iNiR originals once waffle's theme objects are in place.
  - **CRUX / open decision:** quickshell's shared `qs.config` `Appearance.qml` is the Dominance/Dyn one, but iNiR's waffle expects iNiR's `Appearance` (aurora/angel/m3colors). "Byte-for-byte" means waffle needs iNiR's theme objects available WITHOUT breaking Pill (which uses the Dominance `Appearance`). Reconciliation (making Dominance compatible with waffle) is **deferred** and will be done **smartly** later — options to decide then: (a) waffle-scoped theme singleton (`modules/waffle/.../WaffleAppearance.qml`) that iNiR-waffle files import instead of `qs.config`; (b) merge iNiR aurora/angel/m3colors props into the shared `Appearance` behind feature flags; (c) keep two `Appearance` instances by import-scope. NOT to be done blindly.
