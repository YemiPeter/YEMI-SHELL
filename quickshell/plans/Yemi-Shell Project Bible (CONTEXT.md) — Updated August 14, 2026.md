...
- **Waffle (iNiR clone, Niri-focused) — ported 2026-08-18:**
  ...
  - `modules/waffle/settings/` + `waffleSettings.qml` + `modules/settings/WaffleConfig.qml` copied but **PARKED** — standalone launcher needed iNiR-only singletons.
  ...
  - **THEME NOTE — `m3colors` (2026-08-19):** quickshell's `Appearance` uses the **Dominance/`Dyn` pipeline**, NOT iNiR's `m3colors` Material schema. So `MaterialThemeLoader`'s scheme-apply path is a **guarded no-op** (palette `type` persists, but Material variants are not re-skinned). No crash; full visual Material theming is intentionally out of scope. The `WaffleWidgets` color-scheme selector is guarded (`typeof Appearance.m3colors !== "undefined"`) so it can never throw.
