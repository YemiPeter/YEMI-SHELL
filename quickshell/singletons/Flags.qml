...
Singleton {
    ...
    property alias audioAutoMuteDuplicates: adapter.audioAutoMuteDuplicates
    ...
    property string systemMood: "dark"
            ...
            // When two streams play from the same source, mute the older one so
            // the same audio can never stack on itself. OFF: duplicates are
            // only reported in the card, never touched.
            property bool audioAutoMuteDuplicates: false
