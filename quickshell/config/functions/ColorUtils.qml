...
QtObject {
    ...
    /**
     * Converts a color to a hex string in #rrggbb format (exactly 7 characters).
     * Strips the alpha channel if QML yields #aarrggbb (9 chars).
     *
     * @param {color} color - Input color (any Qt.color-compatible string).
     * @returns {string} Hex string in #rrggbb format.
     */
    function colorToHex(color) {
        var hexStr = Qt.color(color).toString();
        // Strip alpha if present: #aarrggbb (9 chars) -> #rrggbb (7 chars)
        if (hexStr.length === 9) {
            hexStr = "#" + hexStr.substr(3);
        }
        return hexStr;
    }
